...
const fetchAccessoriesProducts = async (subcategory?: string) => {
  let query = supabase
    .from('products')
    .select(`
      id, 
...
export default function AccessoriesCategory() {
  ...
  const { 
    ...
    queryKey: ['accessories-products', selectedSubcategory],
    queryFn: () => fetchAccessoriesProducts(selectedSubcategory),
  });
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Complete your look with our premium selection</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex overflow-x-auto pb-4 mb-6 gap-2 no-scrollbar">
          {subcategories.map((subcategory) => (
            <Button
              key={subcategory}
              variant={selectedSubcategory === subcategory || (subcategory === "All Accessories" && !selectedSubcategory) ? "default" : "outline"}
              onClick={() => setSelectedSubcategory(subcategory === "All Accessories" ? undefined : subcategory)}
