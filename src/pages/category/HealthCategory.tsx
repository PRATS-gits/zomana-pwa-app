...
const fetchHealthProducts = async (subcategory?: string) => {
  let query = supabase
    .from('products')
    .select(`
      id, 
...
export default function HealthCategory() {
  ...
  const { 
    ...
    queryKey: ['health-products', selectedSubcategory],
    queryFn: () => fetchHealthProducts(selectedSubcategory),
  });
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="bg-emerald-100 text-emerald-700 h-12 w-12 rounded-full flex items-center justify-center">
              <HeartPulse className="h-6 w-6" />
            </div>
            <div>
              <h1 className="text-3xl font-bold">Health & Wellness</h1>
              <p className="text-muted-foreground">Prioritize your wellbeing with our health products</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex overflow-x-auto pb-4 mb-6 gap-2 no-scrollbar">
          {subcategories.map((subcategory) => (
            <Button
              key={subcategory}
              variant={selectedSubcategory === subcategory || (subcategory === "All Health Products" && !selectedSubcategory) ? "default" : "outline"}
              onClick={() => setSelectedSubcategory(subcategory === "All Health Products" ? undefined : subcategory)}
